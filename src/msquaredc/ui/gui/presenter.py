...
class GUIPresenter(AbstractPresenter):
    ...
    def show_question(self):
        ...
        try:
            ...
            self["question"] = tk.Label(self.frame, text=self.current_question.question, wraplength=self.wraplength, font=("Sans",16,"bold"))
            self["question"].grid(row=0,padx=5,pady=5)
            self["answer"] = tk.Label(self.frame, text=self.current_question.answer, wraplength=self.wraplength, font=("Sans",16,"bold"))
